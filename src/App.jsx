...
function App() {
  ...
  useEffect(() => {
    async function fetchAPIData() {
      ...
      localStorage.clear()
      try {
        const request = await fetch(API_LINK + API_KEY);
  ...
  const gear = <div className="loading-state">
  ...
  return (
    <>
      {data ? <MainImage data={data}/> : gear}
      {data && <Footer handleToggleSideBar={handleToggleSideBar} data={data}/>}
      {sideBarDisplay && <SideBar handleToggleSideBar={handleToggleSideBar} data={data}/> }
    </>
  )
}
